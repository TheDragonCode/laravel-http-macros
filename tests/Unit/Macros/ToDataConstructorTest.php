...
test('simple', function () {
    ...
    expect($response->toData(ConstructorData::class))
...
test('single', function () {
    ...
    expect($response->toData(ConstructorData::class, 'result.item'))
        ->toBeInstanceOf(ConstructorData::class)
        ->id->toBe(1)
        ->title->toBe('Qwerty 1');
});
...
test('many', function () {
    ...
    expect($response->toData(ConstructorData::class, 'result.items.1'))
        ->toBeInstanceOf(ConstructorData::class)
        ->id->toBe(3)
        ->title->toBe('Qwerty 3');
});
...
test('missing key', function () {
    ...
    expect($response->toData(ConstructorData::class, 'missing_key'))->toBeNull();
});
